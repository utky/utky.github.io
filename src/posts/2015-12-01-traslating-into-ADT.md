...
数日経って自分のコードを読み返した時に、そのコードで実現しようとしていることを把握するのに時間がかかるのです。
型として表現されるべき条件がまったく型注釈にあらわれていないのです。
結局関数の定義を見に行くしかなくなってストレスが溜まっていきます。
...
2. 命令を実行する時には`a`を`Store`が解釈できる必要があります。
3. `a`は1つ以上のエンティティeの合成へと変換できる必要があります。
